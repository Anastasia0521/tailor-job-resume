...
- 已有 `姓名_求职决策中心.html` 或 `姓名_单位_岗位_求职决策.html` 时，读取其中 `career-data` 结构化数据，先核对身份、材料范围、论文版本和署名，再复用确认结果。
- 缺少照片时生成无照片版，不使用虚构头像。
...
两个模式均使用 `decision.json` 作为临时构建输入，至少包含 `name`、`title`、非空 `sections` 和 `embedded_data`。`embedded_data` 至少包含 `capability_evidence`、`paper_audit` 和 `career_strategy`；岗位定制模式还必须包含 `job_match`。临时JSON只能存在于任务临时目录，生成HTML并通过校验后不得作为用户可见交付物。

    ...
    for section in data.get("sections", []):
        ...
        for item in section.get("items", []):
            ...
            if item.get("kind") == "paper":
                ...
                doi_html = f'<span class="paper-doi">｜DOI: <a href="https://doi.org/{esc(doi)}">{esc(doi)}</a></span>' if doi else ""
            ...
            items.append(f'<article>{heading}<p>{esc(item.get("body"))}</p></article>')

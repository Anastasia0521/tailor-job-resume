...
def paper_detail(item):
    labels = (("期刊", "journal"), ("等级", "level"), ("作者", "authors"), ("年份", "year"))
    return "　｜　".join(f"{label}：{esc(item.get(key) or '待确认')}" for label, key in labels)
    ...
    for section in data.get("sections", []):
        ...
        for item in section.get("items", []):
            ...
            if item.get("kind") == "paper":
                ...
                heading = f'<h3 class="paper-line"><span class="paper-title" data-full-title="{title}" title="{title}">{title}</span>{doi_html}{meta}</h3>'
                details = paper_detail(item)
                detail_html = f'<p class="paper-detail">{details}</p>' if details else ""
            else:
